...
static mh_stats_t g_hook_stats[32] = {0};  // Support up to 32 hooks
...
// ---- Enable/Disable -------------------------------------------------------
int mh_enable(mh_hook_t *h, bool enable) {
    if (!h || !h->installed) return -1;
    
    if (enable) {
        // Re-apply hook
        uint8_t patch[MH_MAX_PROLOGUE] = {0};
        void* entry = h->user_thunk ? h->user_thunk : h->user_impl;
        emit_abs_jmp_stub(patch, (uint64_t)entry);
        if (h->stolen_len > 14) memset(patch + 14, 0x90, h->stolen_len - 14);
        sys_proc_rw(h->target_addr, patch, h->stolen_len);
    } else {
...
bool mh_is_enabled(mh_hook_t *h) {
    if (!h || !h->installed) return false;
    
    // Check if first bytes match hook pattern
    uint8_t current[6];
    memcpy(current, (void*)h->target_addr, 6);
    return (current[0] == 0xFF && current[1] == 0x25);  // Check for jmp instruction
}

// ---- Statistics -----------------------------------------------------------
mh_stats_t* mh_get_stats(mh_hook_t *h) {
    if (!h || !h->installed) return NULL;
    
    // Find or allocate stats for this hook
    for (int i = 0; i < g_stats_count; i++) {
        if ((void*)&g_hook_stats[i] == h->user_impl) {
            return &g_hook_stats[i];
        }
    }
    
    // Allocate new stats entry
    if (g_stats_count < 32) {
        mh_stats_t* stats = &g_hook_stats[g_stats_count++];
        stats->call_count = 0;
...
// ---- Utilities ------------------------------------------------------------
void mh_hexdump(const void *data, size_t size, const char *label) {
    const unsigned char *p = (const unsigned char*)data;
    char line[128];
    
    if (label) mh_log("[HEXDUMP] %s (%zu bytes):\n", label, size);
