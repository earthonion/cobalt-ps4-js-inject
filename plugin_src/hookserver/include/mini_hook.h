...
/**
 * Enable or disable an installed hook without removing it completely.
 * Useful for temporarily disabling hooks during debugging.
 */
int mh_enable(mh_hook_t *hook, bool enable);

/**
 * Check if a hook is currently active.
 */
bool mh_is_enabled(mh_hook_t *hook);

/**
 * Get statistics about a hook (call count, etc).
 * Returns NULL if hook not installed.
 */
typedef struct {
    uint64_t call_count;
    uint64_t last_call_time;
} mh_stats_t;
mh_stats_t* mh_get_stats(mh_hook_t *hook);
...
#define MH_DEFINE_THUNK(name, cb_fn)                           \
    ...
    __attribute__((naked)) void MH_THUNK_LABEL(name)(void) {           \
        __asm__ volatile(                                              \
            ...
            "mov  %r10, -0x38(%rbp)\n"  /* save r10 */                          \
            "mov  %r11, -0x40(%rbp)\n"  /* save r11 */                          \
            /* If you expect FP args, also save XMM0–XMM5 here with movdqu. */  \
                                                                                \
