...
constexpr uint64_t kMaxReasonableScriptBytes = 1ull << 20;  // 1 MiB

bool layout_reasonable(const StringLikeLayout& layout,
                       uint64_t data_ptr,
                       uint64_t size_val,
                       uint64_t cap_val) {
  if (!layout.valid) {
  ...
  if (!is_canonical_address(data_ptr) || !cap_val || cap_val > kMaxReasonableScriptBytes) {
    return false;
  }
  if (size_val > cap_val || size_val > kMaxReasonableScriptBytes) {
    return false;
  }
  return true;
}
...
extern "C" void my_HTMLScriptExecute(void* self, long arg2, uint64_t arg3,
  ...
  auto read_u64 = [&](size_t off, uint64_t* out) {
    if (!out || off + sizeof(uint64_t) > header.size()) {
      return false;
    }
    std::memcpy(out, header.data() + off, sizeof(uint64_t));
    return true;
  };

  uint64_t remote_data_ptr = 0;
  uint64_t remote_size     = 0;
  uint64_t remote_cap      = 0;
  read_u64(layout.data_off, &remote_data_ptr);
  read_u64(layout.size_off, &remote_size);
  read_u64(layout.cap_off,  &remote_cap);

  bool layout_ok = layout_reasonable(layout, remote_data_ptr, remote_size, remote_cap);
  if (!layout_ok) {
    MH_LOG("[inject.err] reason=layout-unreasonable size=%llu cap=%llu data=%p",
           static_cast<unsigned long long>(remote_size),
  ...
  std::string combined = active_script;
  combined.reserve(active_script.size() + payload.size() + 32);
  ...
  log_preview_bounded(reinterpret_cast<const uint8_t*>(combined.data()),
                      combined.size(), 96);
  MH_LOG("[inject.append] head='%.*s...'", 48, combined.c_str());
  MH_LOG("[inject.debug] header=%p remote_data=%p remote_size=%llu remote_cap=%llu",
         (void*)header_addr, (void*)remote_data_ptr,
         static_cast<unsigned long long>(remote_size),
         static_cast<unsigned long long>(remote_cap));

  bool injected = false;
  if (!combined.empty() && write_payload_to_remote(remote_data_ptr,
                                                   header_addr,
                                                   layout,
                                                   combined,
                                                   remote_cap,
                                                   remote_size)) {
    MH_LOG("[inject.fast] size=%zu cap=%zu", combined.size(),
           static_cast<size_t>(remote_cap));
    g_execute_reentry = true;
    g_real_Execute(self, arg2, arg3, is_external);
    g_execute_reentry = false;
    MH_LOG("[inject.exec] mode=fast");
    injected = true;
  }

  if (!injected) {
    bool layout_span_ok = layout.data_off + sizeof(uint64_t) <= header.size() &&
    ...
    if (!layout_span_ok) {
      ...
      storage->payload.assign(combined.begin(), combined.end());
      ...
      void* cloned_payload = storage->payload.data();
      MH_LOG("[inject.clone] len=%zu header=%p data=%p",
             storage->payload.size(), cloned_header, cloned_payload);
      ...
      g_real_Execute(self,
                     ...
                     is_external);
      g_execute_reentry = false;
      MH_LOG("[inject.exec] mode=clone");
      injected = true;
    }
  }

  if (injected) {
    MH_LOG("[inject] append path ok");
  } else {
    MH_LOG("[inject] append path failed");
  }
}
